...
public class Admin {
	...
	//디폴트 생성자:매개변수 없다!()안에 비어있음
	//생성자 =>[접근한정자] [클래스이름](){}
	//생성자는 반반환값 없다.(void 없다)
	//생성자의 역할은 인스턴스 변수 초기화
	//생성자는 overloading이 가능
	//생성자이름은 무조건 클래스 이름과 동일해야함!
	//new작업을 하려면 적어도 default이상이어야함
	...
	//2.다른 생성자를 호출=>this(~<~,~,~)=>위치는 반드시 생성자에서만 사용
	public Admin(String id, String pwd, String email) {
		this(id,pwd,email,0);//매개변수 4개짜리 호출
}
